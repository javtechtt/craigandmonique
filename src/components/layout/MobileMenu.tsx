...
/**
 * Hamburger trigger + slide-in drawer for the mobile header. The drawer
 * panel is white and uses defensive layering so it always reads as a
 * solid surface (Tailwind bg-white utility, inline backgroundColor, a
 * non-defeasible white linear-gradient, plus `isolation: isolate` to
 * seal the stacking context). No backdrop-filter on the dim layer —
 * mobile Safari occasionally composites siblings as transparent when
 * one carries a filter.
 *
  ...
  return (
    <>
      ...
      {open ? (
        ...
        >
          {/* Backdrop — solid charcoal alpha, no backdrop-filter. */}
          <button
            type="button"
          ...
          {/* Drawer panel — guaranteed opaque white via three channels. */}
          <div
            role="presentation"
